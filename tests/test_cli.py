...
def test_writes_kernel_file(runner, tmp_path):
    out = tmp_path / "gen"
    r = run(runner, tmp_path, MODEL, "--input-shape", "4,64", "--yes", "--out-dir", str(out))
    assert r.exit_code == 0, r.output
...
def test_declining_the_prompt_writes_nothing(runner, tmp_path):
    out = tmp_path / "gen"
    r = runner.invoke(
        main,
    ...
    assert r.exit_code == 0, r.output
    assert not out.exists() or not list(out.glob("*.py"))
...
def test_json_payload(runner, tmp_path):
    ...
    r = run(
        ...
        "--input-shape", "4,64", "--yes", "--out-dir", str(tmp_path), "--json", str(path),
    )
    assert r.exit_code == 0, r.output
...
def test_no_fusable_clusters(runner, tmp_path):
    r = run(runner, tmp_path, NOTHING_TO_FUSE, "--input-shape", "4,64", "--yes")
    assert r.exit_code == 0, r.output
...
def test_several_models_is_a_clean_error(runner, tmp_path):
    r = run(runner, tmp_path, TWO_MODELS, "--input-shape", "4,64", "--yes")
...
def test_untraceable_is_a_clean_error(runner, tmp_path):
    r = run(runner, tmp_path, UNTRACEABLE, "--input-shape", "4,64", "--yes")
    assert r.exit_code != 0
...
def test_bad_dtype_is_a_clean_error(runner, tmp_path):
    r = run(runner, tmp_path, MODEL, "--input-shape", "4,64", "--dtype", "int9", "--yes")
    assert r.exit_code != 0
    assert "Unsupported dtype" in r.output


def test_bad_shape_is_a_clean_error(runner, tmp_path):
    r = run(runner, tmp_path, MODEL, "--input-shape", "4,zzz", "--yes")
    assert r.exit_code != 0
...
def test_apply_rewrites_the_model(runner, tmp_path):
    ...
    r = runner.invoke(main, ["--model", path, "--input-shape", "4,64", "--yes", "--apply"])
    assert r.exit_code == 0, r.output
...
def test_apply_leaves_no_backup_files(runner, tmp_path):
    ...
    path = write(tmp_path, MODEL)
    runner.invoke(main, ["--model", path, "--input-shape", "4,64", "--yes", "--apply"])
...
def test_apply_declining_prompt_leaves_file_alone(runner, tmp_path):
    ...
    r = runner.invoke(main, ["--model", path, "--input-shape", "4,64", "--apply"], input="y\nn\n")
    assert r.exit_code == 0, r.output
